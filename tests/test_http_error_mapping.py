...
from quantclass_sync_internal.constants import REASON_MERGE_ERROR, REASON_NO_DATA_FOR_DATE, STRATEGY_MERGE_KNOWN
from quantclass_sync_internal.http_client import _http_metrics_for_product, _reset_http_metrics, get_download_link, request_data
from quantclass_sync_internal.models import EmptyDownloadLinkError, FatalRequestError, ProductPlan, ProductSyncError, SyncStats
from quantclass_sync_internal.orchestrator import _probe_downloadable_dates
...
class HttpErrorMappingTests(unittest.TestCase):
    ...
    def test_request_data_404_raises_structured_fatal_error(self) -> None:
        with patch("quantclass_sync_internal.http_client.requests.request", return_value=SimpleNamespace(status_code=404)):
            with self.assertRaises(FatalRequestError) as ctx:
                request_data(
                    "GET",
    ...
    def test_request_data_fatal_error_keeps_response_body_preview(self) -> None:
        ...
        with patch("quantclass_sync_internal.http_client.requests.request", return_value=response):
            with self.assertRaises(FatalRequestError) as ctx:
                request_data(
                    "GET",
    ...
    def test_request_data_non_200_retries_close_response_each_attempt(self) -> None:
        ...
        with patch("quantclass_sync_internal.http_client.time.sleep"), patch(
            "quantclass_sync_internal.http_client.requests.request",
    ...
    def test_probe_downloadable_dates_skips_404_and_continues(self) -> None:
        ...
        with patch("quantclass_sync_internal.orchestrator.get_download_link", side_effect=fake_get_download_link):
            result = _probe_downloadable_dates(
    ...
    def test_probe_downloadable_dates_skips_legacy_fatal_without_status(self) -> None:
        def fake_get_download_link(
            ...
            if date_time == "2026-02-07":
                raise FatalRequestError("legacy-no-status")
            return f"https://example.com/{product}/{date_time}.zip"
    ...
    def test_probe_downloadable_dates_401_is_not_silently_skipped(self) -> None:
        ...
        with patch("quantclass_sync_internal.orchestrator.get_download_link", side_effect=fake_get_download_link):
            with self.assertRaises(FatalRequestError) as cm:
    ...
    def test_empty_download_link_raises_specific_exception(self) -> None:
        ...
        with patch("quantclass_sync_internal.http_client.request_data") as mock_req:
            ...
            with self.assertRaises(EmptyDownloadLinkError):
                get_download_link(
                    api_base="http://test",
    ...
    def test_request_data_latest_uses_short_policy(self) -> None:
        ...
        attempts, failures = _http_metrics_for_product("stock-trading-data")
    ...
    def test_process_product_merge_error_reason_raises_product_sync_error(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                ...
                return_value=(SyncStats(skipped_files=1), REASON_MERGE_ERROR),
            ):
            ...
            self.assertEqual(REASON_MERGE_ERROR, cm.exception.reason_code)
    ...
    def test_download_and_prepare_extract_maps_404_from_get_link_to_no_data(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch(
                ...
                side_effect=FatalRequestError("not found", status_code=404),
            ):
                with self.assertRaises(ProductSyncError) as cm:
